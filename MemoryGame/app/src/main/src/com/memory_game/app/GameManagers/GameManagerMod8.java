...
import Bots.EasyBot;
import Bots.HardBot;
import Bots.NormalBot;
import project.*;

public class GameManagerMod8 extends GameManagerNormal{
	static int playerCounter;
	
	public static void InitGameManager(Battle gui,int numberOfPlayersvar,int numberOfBotsvar, int numberOfCardsvar,char... botLvlvar){
	...
	public static void OpenCard(Battle gui, int cardNumber){
	    	if(cardNumber==-1){
	    		moves++;
	    		counter=0;
	    	...
	    	if(counter==1){
	    		...
	    		if(playerCounter==1){
	    			gui.panel_p2.setEnabled(true);
		    		gui.panel_p1.setEnabled(false);
	 ...
	 protected static void PairFound(GUI gui, int x){
	    	switch(x){
	 ...
	 protected static void botPlay(Battle gui){
